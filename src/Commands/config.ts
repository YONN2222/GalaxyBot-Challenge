...
import { ConfigPermissionCheck, GuildCheck } from "../Utils/Checks";

export const data = new SlashCommandBuilder().setName("config").setDescription("Configure the bot");
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  const modal = new ModalBuilder().setCustomId("config-modal").setTitle("Configuration");
...
export async function handleModalSubmit(interaction: ModalSubmitInteraction) {
  ...
  const channels = interaction.fields.getSelectedChannels("incidents-channel", true, [
  ...
  const SuccessTextComponent = new TextDisplayBuilder().setContent("Yay, Configuration saved.");
  const SuccessContainer = new ContainerBuilder().addTextDisplayComponents(SuccessTextComponent);
