...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  // create modal
  const modal = new ModalBuilder().setCustomId("create-incident-modal").setTitle("Create Incident");
...
export async function handleModalSubmit(interaction: ModalSubmitInteraction) {
  ...
  if (!title || !description) {
    await interaction.reply({
      components: [
        new ContainerBuilder()
          .addTextDisplayComponents(
            new TextDisplayBuilder().setContent("Please enter a Title and a Description."),
          )
          .setAccentColor(0xed4245),
      ],
      flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
  ...
  try {
    ...
    await interaction.reply({
      components: [
        new ContainerBuilder()
          .addTextDisplayComponents(
            new TextDisplayBuilder().setContent("An error occurred while creating the incident."),
          )
          .setAccentColor(0xed4245),
      ],
      flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
  ...
  const IncidentContainer = new ContainerBuilder()
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`### #${incident.id} New Incident\n## ${title}`),
    )
  ...
  try {
    ...
    const SuccessfulCreteTextComponent = new TextDisplayBuilder().setContent(
