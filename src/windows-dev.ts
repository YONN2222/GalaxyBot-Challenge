...
const calculateShardList = (clusterID: number) =>
    Array.from({ length: shardsPerCluster }, (_, offset) => clusterID * shardsPerCluster + offset);

const prefixOutput = (clusterID: number, stream: NodeJS.ReadableStream | null | undefined, target: NodeJS.WriteStream) => {
...
const startCluster = (clusterID: number) => {
    ...
    child.on("exit", (code, signal) => {
        ...
        console.log(`[cluster ${clusterID}] exited (${code ?? "null"} ${signal ?? "null"})`);
    });
